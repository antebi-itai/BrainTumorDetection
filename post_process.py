...
def present_all_masks(original_image, gt_mask, grad_mask, hot_masks, cold_masks, title=""):
...
def present_grad_masks(original_image, gt_mask, grad_mask, title=""):
    original_image = original_image.permute(1, 2, 0).cpu().numpy()
    gt_mask = gt_mask.cpu().numpy()
    zeros = np.zeros_like(gt_mask)
    # present GT mask as green
    gt_mask = np.stack((zeros, gt_mask.astype(np.int64), zeros), axis=2)
    # present grad mask as blue
    grad_mask = np.stack((zeros, zeros, grad_mask.astype(np.int64)), axis=2)

    wandb.log({"{title}/grad_masks".format(title=title):
                   [wandb.Image(0.34 * original_image + 0.33 * gt_mask + 0.33 * grad_mask)]})
...
def calc_dice(mask1, mask2):
    intersection = np.logical_and(mask1, mask2)
    DSC = 2 * np.count_nonzero(intersection) / (np.count_nonzero(mask1) + np.count_nonzero(mask2))
    return DSC
